 ...
 */
static int configure(aud_monitor *m, const aud_monitor_config *cfg)
{
  ...
  if (rate != cfg->rate)
  {
     ...
     * accepts whatever the capture side negotiated.
     */
...
aud_monitor *aud_monitor_open(const aud_monitor_config *cfg)
{
  ...
  name = cfg->name != NULL ? cfg->name : AUD_MONITOR_DEFAULT_DEVICE;
  ...
  aud_debug("monitor: %s, %u Hz, %u ch, period %lu frames, buffer %lu frames (%.1f ms)",
            m->name, m->rate, m->channels, (unsigned long)m->period_frames,
