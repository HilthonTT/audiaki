...
/*
 * Which audio system the listing came from. An accessor rather than handing out
 * the ops table, so cmd/devices.c can label a --json row without reaching into
 * the dispatch it is deliberately kept above.
 */
const char *aud_device_backend_name(void)
{
  return aud_backend_capture()->name;
}
